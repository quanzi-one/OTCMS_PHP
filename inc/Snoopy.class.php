...
class Snoopy
{
            ...
            case "http":
                ...
                if ($this->_connect($fp)) {
                    ...
                    if ($this->_framedepth < $this->maxframes && count($this->_frameurls) > 0) {
                        ...
                        while (list(, $frameurl) = OT::NewEach($frameurls)) {
    ...
    function _striplinks($document)
    {
        ...
        while (list($key, $val) = OT::NewEach($links[2])) {
            if (!empty($val))
                $match[] = $val;
        }

        while (list($key, $val) = OT::NewEach($links[3])) {
            if (!empty($val))
                $match[] = $val;
        }
    ...
    function _httprequest($url, $fp, $URI, $http_method, $content_type = "", $body = "")
    {
        ...
        if (!empty($this->rawheaders)) {
            ...
            while (list($headerKey, $headerVal) = OT::NewEach($this->rawheaders))
    ...
    function _prepare_post_body($formvars, $formfiles)
    {
        ...
        switch ($this->_submit_type) {
            case "application/x-www-form-urlencoded":
                reset($formvars);
                while (list($key, $val) = OT::NewEach($formvars)) {
                    if (is_array($val) || is_object($val)) {
                        while (list($cur_key, $cur_val) = OT::NewEach($val)) {
            ...
            case "multipart/form-data":
                ...
                reset($formfiles);
                while (list($field_name, $file_names) = OT::NewEach($formfiles)) {
                    settype($file_names, "array");
                    while (list(, $file_name) = OT::NewEach($file_names)) {
