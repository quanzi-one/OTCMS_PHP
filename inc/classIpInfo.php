...
class IpInfo {
	...
	function __construct($datfile = 'ip.dat'){
		$this->fp=fopen(OT_ROOT .'tools/'. $datfile,'rb');   //二制方式打开
	...
	function getIP() {
		...
		*/
		...
		if ($ip == '::1'){
			...
			@preg_match("/[\d\.]{7,15}/", $ip, $ipArr);
			$ip = $ipArr[0] ? $ipArr[0] : 'unknown';
			unset($ipArr);
		}
		return $ip;
	}
	...
	function getaddress($ip){
		...
		switch (ord($flag)) {
		   ...
		   default: //地区1地区2都没有重定向
			   ...
			   $address['area1']=iconv('GB2312','UTF-8//IGNORE',$this->getstr());
			   $address['area2']=iconv('GB2312','UTF-8//IGNORE',$this->readaddress());
			   break;
		}
...
if ($action=='getip'){
	...
	$myobj=NULL;
	$str="<span class='orange'>".$ip."</span>&ensp;&ensp;来自:".$address["area1"]." ".$address["area2"];
	echo $str;
}
...
if ($action=='queryip'){
	...
	$str="您查询的IP是：<span class='orange'>".$address["ip"]."</span> 来自：".$address["area1"];
	echo $str;
}


function GetIpInfoArr($refIP=''){
	$ipobj = new IpInfo();
	if ($refIP==''){ $refIP=$ipobj->getIP(); }
	$address = $ipobj->getaddress($refIP);

	if (stristr(PHP_OS,'WIN')){
		$infoArr = array(
			'ip'		=> $refIP,
			'city'		=> $address['area1'],
			'address'	=> $address['area1'] .' '. $address['area2'],
			);

	}else{
		$infoArr = array(
			'ip'		=> $refIP,
			'city'		=> iconv('GB2312','UTF-8//IGNORE',$address['area1']),
			'address'	=> iconv('GB2312','UTF-8//IGNORE',$address['area1'] .' '. $address['area2']),
			);
	}
